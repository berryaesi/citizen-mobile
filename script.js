...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sync current location for form auto-fill with shareable code
    function syncCurrentLocation() {
        ...
        showNotification('Getting your location for form auto-fill...', 'info');
        ...
        navigator.geolocation.getCurrentPosition(
            function(position) {
                ...
                // Create a simple shareable code
                const shareCode = `LOC${Math.floor(lat * 10000)}${Math.floor(lng * 10000)}`;
             
                // Store in multiple places
                localStorage.setItem('locationCode', shareCode);
                localStorage.setItem('lastLocationLat', lat);
                localStorage.setItem('lastLocationLng', lng);
                localStorage.setItem('lastLocationAddress', address);
             
                // Show user the code
                showNotification(`Your location code: ${shareCode}`, 'success', 5000);
             
                // Also copy to clipboard
                navigator.clipboard.writeText(shareCode);
             
                hideLocationSyncModal();
            },
            function(error) {
